...
let  config = {
    appid : '5ea671351d224f188aabf28616f1053c',
    token: '73564608918c45f18c4eba7362c854e1',
    uid: null,
    channel: 'livestream'
}
...
let joinStreams = async () => {
    ...
    client.on("volume-indicator", function(evt){
        for (let i = 0; evt.length > i; i++){
            ...
            if(volume > 0){
                document.getElementById(`volume-${speaker}`).src = '{% static \'assets/images/logo.png\' %}'
            }else{
                document.getElementById(`volume-${speaker}`).src = '{% static \'assets/images/logo.png\' %}'
            }
    ...
    let player = `<div class="video-containers" id="video-wrapper-${config.uid}">
                        <p class="user-uid"><img class="volume-icon" id="volume-${config.uid}" src="{% static 'assets/images/logo.png' %}" /> ${config.uid}</p>
...
let handleUserJoined = async (user, mediaType) => {
    ...
    if (mediaType === 'video'){
        ...
        player = `<div class="video-containers" id="video-wrapper-${user.uid}">
                        <p class="user-uid"><img class="volume-icon" id="volume-${user.uid}" src="{% static 'assets/images/logo.png' %}" /> ${user.uid}</p>
